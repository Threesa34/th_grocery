...
module.exports = {
    ...
    getUserDetails: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            conn.query('SELECT *,CONCAT("http://103.252.7.5:8018/uploads/employee/",profilepic) as profilepic FROM `employees` WHERE `id` = '+req.params.userid, function(err, result){
                if(err)
                {
                    res.send({
                        status:0,
        ...
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            connection.acquire(function(err, con){
                ...
                if(userDetails.id != undefined)
                {
                     ...
                     userDetails.profilepic =  userDetails.profilepic.replace(/http:\/\/103.252.7.5:8018\/uploads\/employee\//g, "");
